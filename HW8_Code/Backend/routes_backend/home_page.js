...
router.get('/', async (req, res, next) => {
    ...
    // new_req(url, function (error, response, body) {
    //     parsed_body = JSON.parse(body).results;
    //     var count = 0
    //     var json1 = []
    ...
    //             new_dict.title = parsed_body[i].title;
    //         }
    //         if (parsed_body[i].hasOwnProperty("backdrop_path")) {
    //             new_dict.backdrop_path = "https://image.tmdb.org/t/p/original" + parsed_body[i].backdrop_path;
    //             json1.push(new_dict); // ignore if no poster
    //             count += 1
    //         }
    ...
    // 2. Continue Watching Section - LocalStorage
